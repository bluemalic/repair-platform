...
>
> **超时提醒也不是状态跃迁**：20 待接单 超过接单阈值（默认 24h）时提醒后勤管理员，记一笔 `20 → 20` 的 `ticket_log`（action = `ACCEPT_TIMEOUT`，操作者 = 0 系统）并发通知。**同一工单同一节点只提醒一次**，判重依据就是这笔日志——兜底扫描每分钟都会扫到同一批超期工单，没有判重会把管理员刷屏。
