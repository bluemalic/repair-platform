...
CREATE TABLE `ticket` (
  ...
  `response_minutes` int         DEFAULT NULL COMMENT '响应时长(分钟)，冗余字段用于统计',
  `handle_minutes`  int          DEFAULT NULL COMMENT '处理时长(分钟)，冗余字段用于统计',
...
| 冗余 `response_minutes` / `handle_minutes` | 看板按小时/天聚合时避免每次实时计算，是**用空间换统计性能** |
...
- [ ] 为什么不用外键？
- [ ] 为什么冗余 response_minutes？不用冗余怎么算？
