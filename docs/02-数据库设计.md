...
>
> 本文件是**设计说明**；表结构的**执行真相**是 Flyway 迁移脚本（`src/main/resources/db/migration/`）。
> 改动表结构 = 加一个迁移脚本 + 同步改本文件。
...
erDiagram
    ...
    SYS_USER ||--o{ TICKET_LOG : "操作人（0 = 系统）"
    TICKET ||--o| TICKET_EVALUATION : "验收评价"
    SYS_USER ||--o{ TICKET_EVALUATION : "评价人（学生）"
...
**这张图的画法约定**（改图前先看这三条，否则容易越画越乱）：

1. **只画关系，不画字段**。字段清单在本文 §3 逐表展开，ER 图里再列一遍就是两份会各自过期的事实。
2. **关系 = 真实存在的外键列**，不是"业务上有关联"。所以 `repair_code` 与 `ticket` **之间没有连线**——
   报修码是位置码（楼栋 + 房间），工单通过自己的 `building_id` + `room` 落到同一个位置上，两者靠值相等关联、没有列指向。把这种关联画成外键，会让人以为删码会影响工单。
3. **`ticket_log.operator_id` / `ticket_evaluation.student_id` 指向 `sys_user`**，但 `ticket_log` 里 `operator_id = 0` 表示"系统"（超时调度等无登录态动作），不是真实用户——查操作人姓名时要容忍查不到（当前实现是批量 `selectByIds` 后做映射，查不到就显示空名字，不能因此丢掉这行日志）。

> **`tenant_id` 为什么只连了 4 条线**：它是数据隔离的物理依据，但分布在 **10 张表**上
> （`sys_user` · `sys_role` · `building` · `worker_building` · `ticket_category` · `repair_code` · `ticket` · `ticket_log` · `ticket_evaluation` · `notification`）。
> 全连上会让这张图变成毛线团，所以只保留主干四张，**完整名单以迁移脚本 `V1__baseline.sql` 为准**——
> 判断某张表要不要写 `tenant_id` 条件的规则见 `AGENTS.md` §5 第 6 条。
> 另外三张表刻意**没有** `tenant_id`：`sys_permission`（权限码是全局的）、`sys_user_role`、`sys_role_permission`（关联表，随两端走）。
...
> **`id = 0` 那一行是平台自身，不是学校**：平台运营账号（`sys_user.tenant_id = 0`）靠它登录，
> 因为登录接口是按 `tenantCode` 查这张表定位租户的。`code` 固定是 `platform`，
> 它**不出现在平台运营端的学校列表里**（列表显式排除了 `id = 0`），平台也不能停用它。
> 这与 `sys_role.tenant_id = 0` 表示"平台内置角色"是同一条约定（ADR-012）。

#### `sys_user` 用户（各角色共用一张表）

| 字段 | 类型 | 说明 |
|---|---|---|
| id | bigint | 主键 |
| tenant_id | bigint | 租户ID。**0 表示平台自身**（只有平台运营账号会用它） |
| username | varchar(32) | 登录名（学生用学号、维修工用工号） |
| password | varchar(100) | BCrypt 加密后的密码，**绝不存明文** |
| real_name / phone | varchar(32) / varchar(20) | 姓名、手机号 |
| user_type | tinyint | 1学生 2维修工 3后勤管理 4平台运营 |
| status | tinyint | 1启用 0停用 |
| must_change_password | tinyint | 是否需强制改密 0否 1是。管理员建的账号为 1，改密成功后置 0 |
| deleted / create_time / update_time | —— | 审计字段 |

索引：`uk_tenant_username(tenant_id, username)`、`idx_tenant_type(tenant_id, user_type)`。

**设计说明**：三个租户角色**共用一张表**，靠 `user_type` 区分。不拆三张表的理由是登录逻辑、密码校验、token 管理完全一致，拆表只会产生三份重复代码；差异化的权限走 `sys_role`。

> `user_type = 4`（平台运营）是唯一的例外：它**不属于任何租户**，`tenant_id` 固定为 0。
> 它没有创建接口——只能由环境变量引导（见 docs/05 §4.3），且被拦截器关在 `/api/platform/**` 里（ADR-012）。

#### `sys_role` 角色 / `sys_permission` 权限点

| 表 | 关键字段 | 说明 |
|---|---|---|
| `sys_role` | id, tenant_id, code, name, description, status, deleted, 审计 | `tenant_id = 0` 表示**平台内置角色**（所有租户共用）；唯一键 `uk_tenant_code(tenant_id, code)`。种子数据里有 4 个：`STUDENT` / `WORKER` / `ADMIN` / `PLATFORM` |
| `sys_permission` | id, code, name, type, parent_id, sort, status, 审计 | 权限是**全局的**（不区分租户）；`type` 1菜单 2按钮/接口；`code` 形如 `ticket:dispatch` |

**设计说明**：权限点不做租户隔离——功能本身是平台定义的，租户能决定的只是"给哪个角色"。这避免了每个租户都要复制一份权限表。`parent_id` 预留树形层级，当前 22 个权限点都是叶子节点。
...
**设计说明**：两张表结构几乎一样，都是**租户内的字典数据**，各自带 `status` + `deleted`。

- **"下线一项"用 `status=0`（停用），不用删除**：停用的楼栋 / 类别不能再用于新报修（`submit` 会校验"存在 + 同租户 + 启用"），而历史工单照常显示它的名称
- `deleted` 只留给"**建错了、从没被引用过**"的项。原因是全局逻辑删除会把软删行从**所有**查询里滤掉——`TicketServiceImpl` 补名称用的 `selectByIds` 也在内，所以删掉一个被引用的楼栋不会报错，只会让引用处**静默显示空名称**（历史工单的楼栋名、学生扫码拿到的楼栋名）。接口层因此加了"有引用就不许删"的前置校验，见 `docs/03` §5.4
- **名称在租户内不重复由应用层保证**：这两张表都没有名称唯一索引，因为 `uk_tenant_name` 会和逻辑删除打架（软删之后同一名称就再也建不回来）
- `ticket_category.default_urgency` 让"水电"默认紧急、"家具"默认普通，减少学生填写负担
...
索引：`uk_ticket(ticket_id)`、`idx_student(student_id)`、`idx_create_time(create_time)`。
...
| `idx_status_dispatch`（ticket） | **超时兜底扫描**：`status = 20/30 AND dispatch_time < 阈值`。兜底是系统上下文（无租户条件），用不上 `idx_tenant_status` 的前导列 |
| `idx_create_time`（ticket_evaluation） | **验收超时兜底**：`create_time < 阈值` 找评价超期且工单仍为 50 的记录 |
...
**注意最左前缀原则**：`idx_tenant_status(tenant_id, status, submit_time)` 能服务 `(tenant_id)`、`(tenant_id, status)`、`(tenant_id, status, submit_time)` 三种查询，但**不能**服务单查 `status`——超时兜底扫描正是"单查 status + dispatch_time"这种形状，所以另建了 `idx_status_dispatch`。

**兜底扫描的查询形状与成本**（ADR-001 / ADR-006 的实现约束）：三个兜底扫描都是"系统上下文"（无租户条件），且必须带**状态过滤 + 单轮 LIMIT(200)**——少了状态过滤，已处理的工单会被每分钟重复扫到，结果集只增不减；少了 LIMIT，一次会把历史上所有超期工单拉进内存。评价那条还要 JOIN `ticket` 限定 `status = 50`，见 `resources/mapper/TicketEvaluationMapper.xml`。
