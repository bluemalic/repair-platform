...
| — | 管理端基础数据接口：**维修工管理（含负责楼栋）** · 报修码管理 · 楼栋 · 类别 | 🚧 维修工已实现；楼栋 / 类别 / 报修码未开始（接口已在 docs/03 定义） |
...
> 测试与验证：`mvn -B clean package` 本地全绿（当前 60 个测试，覆盖状态机流转、幂等、数据权限、超时调度、统计口径、限流边界、接口错误契约、维修工管理）；
