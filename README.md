...
| M5 | 前端 · 后勤管理端（Vue 3 + Element Plus + ECharts）：登录 / 工单（派单选人 + 详情抽屉）/ 通知（未读角标）/ 统计看板 / 维修工 · 学生 · 报修码 · 楼栋 · 类别 | ✅ 已完成 |
...
> 测试与验证：`mvn -B clean package` 本地全绿（当前 116 个测试，覆盖状态机流转、幂等、数据权限、超时调度、统计口径、限流边界（报修码按用户 / 登录按账号）、接口错误契约、自助改密、首登强制改密、演示数据重置、维修工管理、学生账号管理、字典管理、报修码管理、文件上传）；
