...
| M5 | 前端 · 后勤管理端（Vue 3 + Element Plus + ECharts）：登录 / 工单列表与**派单选人** / 统计看板 / 维修工·报修码·楼栋·类别四个管理页 | 🚧 页面已就绪，还差工单详情抽屉与通知页 |
...
> 测试与验证：`mvn -B clean package` 本地全绿（当前 78 个测试，覆盖状态机流转、幂等、数据权限、超时调度、统计口径、限流边界、接口错误契约、维修工管理、字典管理、报修码管理）；
