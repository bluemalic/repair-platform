...
| M4 | AI 数据助手 + SQL 安全网关（只读账号 / 白名单 / 强制数据权限 / 熔断） | 🚧 进行中：**SQL 安全网关已落地并有 27 条单测**（纯逻辑、不依赖模型）；只读账号、模型接入、管理端页面在后继 PR（ADR-003、ADR-005） |
...
> 测试与验证：`mvn -B clean package` 本地全绿（当前 157 个测试，覆盖状态机流转、幂等、数据权限、超时调度、统计口径、限流边界（报修码按用户 / 登录按账号）、接口错误契约、自助改密、首登强制改密、演示数据重置、维修工管理、学生账号管理、租户入驻与平台域收口、**AI SQL 安全网关（注入/多语句/DDL/白名单/敏感列/写文件 27 条）**、字典管理、报修码管理、文件上传）；
