...
| M4 | AI 数据助手 + SQL 安全网关（只读账号 / 白名单 / 强制数据权限 / 熔断） | ✅ 已完成：四层闸门 + 模型接入 + 两个接口（含 SSE 流式）+ 管理端「AI 问数」页；**20 问评测集 39/40 符合预期**（[docs/07](docs/07-AI评测集.md)） |
...
| P1 | 工单转派 · Excel 导出 · 操作审计 · 压测报告 | 🚧 未开始 |

> 测试与验证：`mvn -B clean package` 本地全绿（当前 191 个测试，覆盖状态机流转、幂等、数据权限、超时调度、统计口径、限流边界（报修码按用户 / 登录按账号）、接口错误契约、自助改密、首登强制改密、演示数据重置、维修工管理、学生账号管理、租户入驻与平台域收口、**AI SQL 安全网关（33 条：白名单 / 子查询 / 注入落点 / 敏感列）与问数链路（28 条：只读账号不能写、超时中断、截断、越权被收窄、结论降级、接口权限与限流档位、SSE 帧与错误帧、数字序列化）**、字典管理、报修码管理、文件上传）；
...
自然语言 → Schema 注入 → SQL 生成 → **安全网关（四层）** → 只读执行 → 表格 + 图 + 结论。

- **四层闸门**（[ADR-003](docs/04-架构决策记录.md)）：① 只读数据库账号（应用启动时按白名单授权，口令随环境变量同步）
  ② SQL 白名单（单条 `SELECT`，禁 DDL/DML/多语句/UNION/WITH/**子查询**、禁敏感列）
  ③ **强制租户条件**（在 AST 上给每张表注入 `tenant_id`——JOIN 进来的表注到 `ON`）
  ④ 超时 3 秒中断 + 行数上限 1000 行截断
- **流式**：`GET /api/ai/query/stream` 只流**阶段**（`sql` → `data` → `done`），前端用 `fetch` 读流（[ADR-013](docs/04-架构决策记录.md)）
- **评测集**：20 问自然语言 → 参考答案，手动跑、要花钱、不进 CI；准确率与错例见 [docs/07](docs/07-AI评测集.md)
...
| [AI 问数评测集](docs/07-AI评测集.md) | 20 问准确率、错例、评测发现的问题 |
