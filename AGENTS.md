...
| `docs/07-AI评测集.md` | 改动 AI 的提示词 / 安全网关 / 模型之后 —— 20 问的准确率和已踩过的坑都记在这里（手动跑，要花钱，不进 CI） |
...
- **改表结构 = 加一个迁移脚本**：`src/main/resources/db/migration/V{n}__xxx.sql`（Flyway 在应用启动时执行），
  并同步改 `docs/02-数据库设计.md`。迁移脚本是表结构的**执行真相**，`docs/02` 是设计说明；两者不一致时以脚本为准并回去改文档。
  新增基础数据（如权限码）改 `R__seed.sql`——它是可重复执行的，改完下次启动自动重跑。
...
- **登录接口的限流不走注解，走 Service 层**（`AuthServiceImpl` + `RateLimiter`），维度是**租户 + 账号**、默认 10 次/分钟：账号在请求体里，拦截器读不到（读了 Controller 就读不到了）；而按 IP 计在校园网里会误伤——宿舍 / 机房的 NAT 出口后面成百上千人共用一个公网 IP。这是本项目唯一一处"限流不在拦截器里"的地方，**改动它之前先读 `RateLimiter` 的类注释**。
...
- **文档与代码必须同一个 PR，不允许"下个 PR 补"**。写完代码、跑绿测试之后，对着这张表逐条过一遍：

  | 改了什么 | 同一个 PR 里要改的文档 |
  |---|---|
  | 表结构 / 字段 / 索引 | 新增迁移脚本 + `docs/02-数据库设计.md` |
  | 基础数据（权限码等） | `R__seed.sql` + `docs/03-接口规范.md` |
  | 接口 / 错误码 | `docs/03-接口规范.md` |
  | 部署方式 / 环境变量 | `docs/05-部署文档.md`、`.env.example` |
  | 技术选型 / 架构取舍 | `docs/04-架构决策记录.md` 加一条 ADR |
  | 模块职责 / 数据流 | `docs/06-架构图.md` |
  | 进度 / 测试数 / 待办 | `README.md`、`docs/00-项目全景.md` |

  **进度与测试数是每个功能 PR 的收尾动作**，不是"攒一批再更新"——文档落后于代码时，
  它的每一条结论都要重新验证才能相信，等于没有。
